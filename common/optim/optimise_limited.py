...
class ParameterProcessor:
    ...
    def save_delta_current(self, momentum, weight_decay = 0.0, calc_norm2_squared=False): #, eta, eta_scale = 1.0
        #eta_w = eta * eta_scale
        with torch.no_grad():
            norm2_squared = 0.0
...
class StepResult:
    def __init__(self, eta, ck_armiho=0.0, pq_norm=0.0, qq_norm=0.0, cos_phi=0.0, kappa_avg=1.0, kappa_min=1.0\
                 , grad_norm2_squared=None, accum_norm2_squared=None, cos_phi_sample=None, ratio_sample=None):
        self.eta = eta
...
class NetLineStepProcessorAbstract:
    def __init__(self, net, meta, device):
        ...
        self.do_lr_on_sample = False #Is lr estimated on sample
        ...
        self.big_m = 0.0 #Lipschitz constant for loss-gradient/hessian
    ...
    def eta_analytic_n2_onehot(self, eta_test, delta_pq, delta_qq, norm_pq, norm_qq, denom_addend):
        with torch.no_grad():
            cos_phi = torch.sum(delta_pq*delta_qq)/(norm_pq*norm_qq + self.epsilon)
            eta_next = self.alpha*norm_pq*cos_phi*eta_test/torch.maximum(norm_qq+denom_addend, self.beta_min)
            if self.do_logging:
                logging.info("##cos(pp^qq)={}, norm_pq={}, norm_qq={}, denom_addend={}, eta_test={}, eta_next={}, alpha={}, beta_min={}"\
                            .format(cos_phi, norm_pq, norm_qq, denom_addend, eta_test, eta_next, self.alpha, self.beta_min))
...
class NetLineStepProcessor(NetLineStepProcessorAbstract):
    ...
    def step(self, labels, images, momentum = 0.0, weight_decay = 0.0, sample_labels=None, sample_images=None):
        ...
        with torch.no_grad():
            ...
            if self.do_lr_on_sample == False:
                #logits1 = self.do_forward(images, False)
                qq1 = self.softmax(self.do_forward(images, False))
                delta_pq, delta_qq1 = pp-qq0, qq1-qq0
            else:
                qq1_sample = self.softmax(self.do_forward(sample_images, False))
                delta_pq, delta_qq1 = pp_sample-qq0_sample, qq1_sample-qq0_sample

            logging.info("##Tmp: Before eta_analytic_n2")
            norm_pq, norm_qq1 = norm(delta_pq, ord='fro'), norm(delta_qq1, ord='fro')
            denom_addend = -grad_norm2_squared*self.big_m*torch.sum(delta_pq)
            eta2, cos_phi = self.eta_analytic_n2_onehot(self.eta1, delta_pq, delta_qq1, norm_pq, norm_qq1, denom_addend)
            ...
            accum_norm2_squared = self.paramProcessor.save_delta_current(momentum, weight_decay, self.do_calc_grad_norm2)
            logging.info("##Tmp:finish")
            return StepResult( eta_curr, ck1_armiho, norm_pq, norm_qq1, cos_phi, kappa_avg, kappa_min\
                              , grad_norm2_squared, accum_norm2_squared, cos_phi_sample, ratio_sample)
